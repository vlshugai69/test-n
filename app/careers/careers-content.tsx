  ...
  const openPositions = [
    {
      ...
      salary: "$150k – $250k",
      description: "We are seeking a product-oriented CTO with deep experience in AI and ML to join us at the forefront of a pivotal mission. As a key member of our founding team, your role goes beyond technical mastery; you will be a driving force in using AI, ML and other advanced technologies to reshape the way we consume news.",
      details: {
        shares: "5% – 15% Equity",
    ...
    <main className="w-full">
      ...
      <section className="h-[493px] relative overflow-hidden">
        <div className="w-full h-[840px] left-0 top-0 absolute">
          ...
          <div className="w-full h-[840px] left-0 top-0 absolute mix-blend-soft-light bg-cover bg-center bg-[url('/images/hero-background.png')]" />
        </div>
        ...
        <div className="absolute left-1/2 top-[180px] -translate-x-1/2 w-full px-4 md:px-8 max-w-[873px] flex flex-col items-center gap-4">
          ...
          <p className="text-center text-[#c1c3cc] max-w-[873px] text-[18px] sm:text-[20px] md:text-[24px] lg:text-[28px] font-normal font-sf-pro-display leading-[1.4]">
            We strive to create an inclusive environment where every voice is heard and ideas are fostered.
          </p>
        </div>
      </section>
            ...
            >
              {hoveredCard === index && (
                <div
                  ...
                  style={{
                    backgroundImage: "url('/images/components/small-background-logo.svg')",
              ...
              <div className="flex flex-col gap-8 flex-1 mb-8 relative z-10">
                  ...
                  {[
                    ...
                    <div
                      ...
                      className={`h-[40px] px-[16px] rounded-[32px] border ${
                        hoveredCard === index ? 'border-white' : 'border-[#923dfb]'
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <p className={`text-[17px] font-normal font-sf-pro-display leading-relaxed ${
                    hoveredCard === index ? 'text-white' : 'text-[#c1c3cc]'
                  }`}>
                    {position.description}
                  </p>
                  <div className={`text-[17px] font-normal font-sf-pro-display leading-relaxed ${
                    hoveredCard === index ? 'text-white' : ''
                  }`}>
                    {!position.highlighted && <span className="text-[#c1c3cc]">Shares: </span>}
                    {position.details.shares}<br/>
                    {!position.highlighted && <span className="text-[#c1c3cc]">Location: </span>}
                    {position.details.location}<br/>
                    {!position.highlighted && <span className="text-[#c1c3cc]">Preferred Timezones: </span>}
                    {position.details.timezone}<br/>
                    {!position.highlighted && <span className="text-[#c1c3cc]">Visa Sponsorship: </span>}
                    {position.details.visa}
                  </div>
                </div>
              </div>

              <Link href={`/careers/positions/${position.title.toLowerCase().replace(/\s+/g, '-')}`}>
                <div className={`inline-flex h-[52px] px-11 py-[18px] ${
                  hoveredCard === index ? 'bg-white' : 'bg-[#35354b]'
                } rounded-[100px] items-center justify-center relative z-10`}>
                  <div className={`text-[16px] font-medium font-sf-pro-display leading-none ${
